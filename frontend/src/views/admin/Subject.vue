...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Code", value: "SubjectCode" },
        { text: "Faculty", value: "FacultyId.FacultyName" },
  ...
  methods: {
    ...
    editItem(item) {
      ...
      this.name = item.SubjectName;
      this.code = item.SubjectCode;
      this.dialog = true;
    },
    ...
    async deleteItemConfirm() {
      try {
        const token = localStorage.getItem("token");
        await axios({
          url: `http://localhost:3000/subject/delete?id=${this.editedItem._id}`,
          method: "DELETE",
          headers: {
            "auth-token": token,
          },
    ...
    async save() {
      try {
        ...
        let user = {
          name: this.name,
          code: this.code,
          facultyId: this.selectFaculty,
        };
        if (this.isAddItem == false) {
          await axios({
            url: `http://localhost:3000/subject/update?id=${this.editedItem._id}`,
            data: user,
            method: "PUT",
            headers: {
              "auth-token": token,
          ...
          await axios({
            url: `http://localhost:3000/subject/add`,
            data: user,
            method: "POST",
            headers: {
              "auth-token": token,
